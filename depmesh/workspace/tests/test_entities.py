...
class TestWorkspace:
    def test_relations_by_forward_id__indexes_relations_by_id(self, tmp_path: Path) -> None:
        relation = Relation(id="tests", reverse_id="tested_by")
        workspace = Workspace(root=tmp_path, relations=(relation,))

        assert workspace.relations_by_forward_id == {"tests": relation}

    def test_relations_by_backward_id__indexes_relations_by_reverse_id(self, tmp_path: Path) -> None:
        relation = Relation(id="tests", reverse_id="tested_by")
        workspace = Workspace(root=tmp_path, relations=(relation,))
