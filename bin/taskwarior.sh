...
case "${ARGS[0]}" in
    add | log)
        exec task rc:.taskrc rc.confirmation:no "${OVERRIDES[@]}" "${ARGS[@]}" project:depmesh "logged_at:$(logged_at)" "logged_time:$(logged_time)"
        ;;
esac

exec task rc:.taskrc rc.confirmation:no "${OVERRIDES[@]}" project:depmesh "${ARGS[@]}"
